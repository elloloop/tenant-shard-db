...
def _extract_node(msg, node_opt_num: int, field_opt_num: int) -> NodeInfo | None:
    ...
    # Parse the NodeOpts message from the raw bytes
    from google.protobuf.compiler import plugin_pb2  # noqa: F401

    # Import our options proto
    ...
    acl = node_opts.get("acl", {})

    return NodeInfo(
        type_id=node_opts["type_id"],
        name=msg.name,
        fields=fields,
        acl_public=acl.get("public", False),
        acl_tenant_visible=acl.get("tenant_visible", True),
        acl_inherit=acl.get("inherit", True),
...
def _extract_edge(msg, edge_opt_num: int, field_opt_num: int) -> EdgeInfo | None:
    ...
    return EdgeInfo(
        ...
        from_type=edge_opts.get("from_type", 0),
        to_type=edge_opts.get("to_type", 0),
        props=props,
...
def _parse_node_opts(raw: bytes) -> dict[str, Any]:
    ...
    while pos < len(raw):
        ...
        if field_number == 1 and wire_type == 0:
            ...
            acl_bytes = raw[pos : pos + length]
            result["acl"] = _parse_acl_opts(acl_bytes)
            pos = pos + length
        elif field_number == 3 and wire_type == 2:
            length, pos = _decode_varint(raw, new_pos)
            result["description"] = raw[pos : pos + length].decode("utf-8")
            pos = pos + length
...
def _parse_acl_opts(raw: bytes) -> dict[str, Any]:
    """Parse AclOpts from raw bytes."""
    result: dict[str, Any] = {}
    pos = 0
    while pos < len(raw):
        tag, new_pos = _decode_varint(raw, pos)
...
def _parse_edge_opts(raw: bytes) -> dict[str, Any]:
    ...
    Field numbers (from entdb_options.proto):
      1: edge_id (varint)
      2: name (string)
      3: from_type (varint)
      4: to_type (varint)
      5: propagate_share (bool/varint)
      6: unique_per_from (bool/varint)
      7: description (string)
      8: deprecated (bool/varint)
    """
    result: dict[str, Any] = {}
    pos = 0
    while pos < len(raw):
...
def generate_python(nodes: list[NodeInfo], edges: list[EdgeInfo]) -> str:
    ...
    lines = [
        ...
        "",
        "from entdb_sdk import AclDefaults, EdgeTypeDef, NodeTypeDef, field",
        "",
    ]
    ...
    for n in nodes:
        ...
        if not n.acl_tenant_visible:
            acl_parts.append("tenant_visible=False")
        if not n.acl_inherit:
            acl_parts.append("inherit=False")
        if acl_parts:

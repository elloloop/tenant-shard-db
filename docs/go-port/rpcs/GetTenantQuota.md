...
> Implementation: `server/go/internal/api/get_tenant_quota.go`. The Python-source citations
> below are historical (Python server was retired in EPIC #407 Phase 4D,
> commit `8d07f5f`). See ADR-016 for the write-path contract this RPC
> follows.

EPIC #407. Reference Python: `server/go/internal/api/get_tenant_quota.go`.
...
- **Phase 1 — monthly write quota (durable, calendar-month):**
  ...
  - `int64 period_end_ms = 5` — **computed locally** as `_next_calendar_month_start_ms()`; do NOT read from DB. This keeps dashboards correct for tenants with no writes this period (`server/go/internal/api/get_tenant_quota.go`).
...
- Gate: `_require_admin_or_owner(tenant_id, request.actor, ctx, "GetTenantQuota")` (`server/go/internal/api/get_tenant_quota.go`, definition `:2656`). The Python helper resolves the **trusted** actor from interceptor metadata (session / JWT / API-key) and rejects callers who are not `admin` or `owner` on the tenant.
...
- global_store reads only:
  - `get_quota_config(tenant_id)` ⇒ `tenant_quotas` row (`server/go/internal/globalstore/`). Returns `None` for unknown tenants.
  - `get_usage(tenant_id)` ⇒ `tenant_usage` row (`server/go/internal/globalstore/`). Returns a synthesized `{period_start_ms: _calendar_month_start_ms(), writes_count: 0}` for tenants with no row yet (`server/go/internal/globalstore/`).
...
|---|---|---|
| `OK` | normal path; unknown tenant returns zero-valued response | `server/go/internal/api/get_tenant_quota.go`; (legacy Python unit test, removed in Phase 4D) |
| `INVALID_ARGUMENT` | `request.tenant_id == ""` | `server/go/internal/api/get_tenant_quota.go`; `tests/python/integration/test_grpc_contract.py:604-608` |
| `UNIMPLEMENTED` | server started without a `global_store` (quota registry not configured) | `server/go/internal/api/get_tenant_quota.go` |
| `PERMISSION_DENIED` | trusted actor lacks admin/owner on tenant; covers wire-claimed-admin escalation | `server/go/internal/api/get_tenant_quota.go`; `tests/python/integration/test_privilege_escalation.py:374-378`, `test_grpc_contract.py:609-613` |
| `UNAUTHENTICATED` | no trusted identity (interceptor) | `auth/` interceptor, before handler |
| `INTERNAL` | unexpected exception (logged, re-raised) | `server/go/internal/api/get_tenant_quota.go` |
...
- `internal/globalstore/` — `GetQuotaConfig(ctx, tenantID) (*Config, error)`, `GetUsage(ctx, tenantID) (*Usage, error)` (must return zero-valued Usage with current month-start when no row exists, mirroring `server/go/internal/globalstore/`).
...
- (legacy Python unit test, removed in Phase 4D) — happy path: returns config + usage, all 9 numeric fields and `hard_enforce` populated; `period_start_ms == _calendar_month_start_ms()`, `period_end_ms == _next_calendar_month_start_ms()`.
- (legacy Python unit test, removed in Phase 4D) — unknown tenant: zero-valued response, `period_end_ms` still computed.
- (legacy Python unit test, removed in Phase 4D) — non-admin caller: handler propagates the admin-gate's `PermissionError`.
- (legacy Python unit test, removed in Phase 4D) — `period_end_ms` is strictly after `now` and strictly after `period_start_ms` (calendar-month math invariant).
...
Notes:
- All `int64`/`int32` are emitted as 0 when source values are missing, matching Python `int(... or 0)` coalescing (`server/go/internal/api/get_tenant_quota.go`).

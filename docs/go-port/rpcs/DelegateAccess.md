...
> Implementation: `server/go/internal/api/delegate_access.go`. The Python-source citations
> below are historical (Python server was retired in EPIC #407 Phase 4D,
> commit `8d07f5f`). See ADR-016 for the write-path contract this RPC
> follows.

EPIC #407. Source of truth: handler at `server/go/internal/api/delegate_access.go`;
WAL helper at `server/go/internal/api/admin_handlers.go`;
storage (legacy direct-write) at `server/go/internal/store/`.
...
|------|-----|------|------|
| `actor` | 1 | `string` | Caller identity. **Untrusted** — overridden by the AuthInterceptor's authoritative actor (`server/go/internal/api/delegate_access.go`). |
| `tenant_id` | 2 | `string` | Required (`server/go/internal/api/delegate_access.go`). |
...
time** (`server/go/internal/api/delegate_access.go`) — a SELECT before the WAL event is
applied, so an estimate, not a post-apply count. Pinned by
`test_admin_ops.py:438`. `expires_at` echoes request; `0` when permanent
(`server/go/internal/api/delegate_access.go`). SDK consumers depend on this shape
(`sdk/go/entdb/admin_test.go:291-333`).

## Auth (delegator must hold the right; trusted-actor)

1. `_check_tenant(tenant_id)` (`server/go/internal/api/delegate_access.go`, at `:2755`) —
...
3. `_require_admin_or_owner(...)` (`server/go/internal/api/delegate_access.go`, at `:2762`)
...
**KNOWN GAP — fix in Go port.** `apply/server/go/internal/apply/applier.go` only
...
(`server/go/internal/store/`). Consider a typed `grant_kind` column in v2.
...
| `OK` + `success=false, error=...` | Catch-all for non-gRPC exceptions (`server/go/internal/api/delegate_access.go`). |
...
| `PERMISSION_DENIED` (abort) | Caller is not admin/owner (`server/go/internal/api/delegate_access.go`). Pinned by `test_admin_operations.py:538-557`. |
...
- `store` — `CountNodesByOwner(tenantID, owner) (int, error)` for `delegated` pre-count (`server/go/internal/api/delegate_access.go`).
...
- **`ShareNode`** (`server/go/internal/api/delegate_access.go`) — per-node share; uses
...
- **`TransferUserContent`** (`server/go/internal/api/delegate_access.go`) — same WAL-first
  pattern via `handle_transfer_user_content`. Its `admin_transfer_content`
  branch (`server/go/internal/apply/applier.go`) is the template for the missing
  `admin_delegate_access` branch.
- **`RevokeAllUserAccess`** — `admin_revoke_access` branch
  (`server/go/internal/apply/applier.go`). **Does NOT target delegations** — deletes from
...
- **`ExecuteAtomic`** — WAL envelope template (`server/go/internal/api/delegate_access.go`).

## Contract tests pinning behavior (file:line)

- (legacy Python unit test, removed in Phase 4D) — WAL-first: handler
...
- (legacy Python unit test, removed in Phase 4D) — empty `permission`
  defaults to `"read"` in the WAL payload.
- (legacy Python unit test, removed in Phase 4D) — bulk grant via
  direct path: 2 alice-owned nodes -> bob via `list_shared_with_me`;
  carol-owned excluded. Go applier branch must reproduce.
- (legacy Python unit test, removed in Phase 4D) — `expires_at`
  persisted; permission stored verbatim.
- (legacy Python unit test, removed in Phase 4D) — permanent
  delegation -> `expires_at IS NULL`.
- (legacy Python unit test, removed in Phase 4D) — audit row
  `action="delegate_access"`, metadata has `from_user`, `to_user`,
  `permission`, `delegated`.
- (legacy Python unit test, removed in Phase 4D) — past `expires_at`
  filtered at read time.
- (legacy Python unit test, removed in Phase 4D) — handler happy
  path; `delegated=2`.
- (legacy Python unit test, removed in Phase 4D) — non-admin ->
  `PERMISSION_DENIED`.
- (legacy Python unit test, removed in Phase 4D) — default
  `"read"` reaches the WAL event.
- (legacy Python unit test, removed in Phase 4D) — empty `to_user`
  -> `INVALID_ARGUMENT`.

...
class TestAclBenchmarks:
    @pytest.mark.benchmark(group="acl")
    def test_can_access_point_check(self, store, benchmark):
        """Benchmark: single can_access point check."""
        nid = _node(store, owner=ALICE)
        _share(store, nid, BOB, "read")
        benchmark(store._sync_can_access, TENANT, nid, [BOB])

    @pytest.mark.benchmark(group="acl")
    def test_can_access_inherited(self, store, benchmark):
        """Benchmark: can_access with 3-level inheritance chain."""
        root = _node(store, type_id=1)
        mid = _node(store, type_id=2)
        leaf = _node(store, type_id=3)
        _edge(store, root, mid, propagates_acl=True)
    ...
    @pytest.mark.benchmark(group="acl")
    def test_can_access_with_groups(self, store, benchmark):
        """Benchmark: can_access with group resolution."""
        nid = _node(store, owner=ALICE)
        _group_add(store, "group:eng", BOB)
        _share(store, nid, "group:eng", "read")
        groups = store._sync_resolve_actor_groups(TENANT, BOB)
    ...
    @pytest.mark.benchmark(group="acl")
    def test_resolve_actor_groups(self, store, benchmark):
        """Benchmark: resolve group memberships."""
        _group_add(store, "group:eng", BOB)
    ...
    @pytest.mark.benchmark(group="acl")
    def test_connected_nodes_10_children(self, store, benchmark):
        """Benchmark: get_connected_nodes with 10 children."""
        parent = _node(store, owner=ALICE)
        for _ in range(10):
            c = _node(store, type_id=2)
            _edge(store, parent, c, edge_type_id=30, propagates_acl=True)
        _share(store, parent, BOB, "read")
        benchmark(store._sync_get_connected_nodes, TENANT, parent, 30, [BOB], 100, 0)

    @pytest.mark.benchmark(group="acl")
    def test_connected_nodes_100_children(self, store, benchmark):
        """Benchmark: get_connected_nodes with 100 children."""
        parent = _node(store, owner=ALICE)
        for _ in range(100):
            c = _node(store, type_id=2)
            _edge(store, parent, c, edge_type_id=31, propagates_acl=True)
        _share(store, parent, BOB, "read")
        benchmark(store._sync_get_connected_nodes, TENANT, parent, 31, [BOB], 100, 0)

    @pytest.mark.benchmark(group="acl")
    def test_shared_with_me_50_nodes(self, store, benchmark):
        """Benchmark: list_shared_with_me with 50 shared nodes."""
        for _ in range(50):
            nid = _node(store, owner=ALICE)
            _share(store, nid, BOB, "read")

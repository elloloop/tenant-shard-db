...
def stop_all():
    ...
    for name in [
        "bench-entdb", "bench-rp0", "bench-rp1", "bench-rp2",
        "bench-minio", "bench-minio-init", "bench-pg",
    ]:
...
def start_entdb(tier: dict, acks: str = "all"):
    ...
    subprocess.run([
        ...
        "-e", "WAL_BACKEND=kafka",
        "-e", f"KAFKA_BROKERS={brokers}",
        "-e", f"KAFKA_TOPIC={topic}",
        "-e", f"KAFKA_CONSUMER_GROUP=bench-applier-{acks}",
        "-e", f"KAFKA_ACKS={acks}",
        "-e", f"KAFKA_ENABLE_IDEMPOTENCE={'true' if acks == 'all' else 'false'}",
        "-e", "APPLIER_BATCH_SIZE=10",
        ...
        "-e", "LOG_LEVEL=WARNING",
        "-e", "LOG_FORMAT=text",
        "-e", "SCHEMA_FILE=/app/schema.yaml",
        "-v", "/Users/arun/projects/opensource/tenant-shard-db/schema.yaml:/app/schema.yaml:ro",
        "entdb-server-bench",
    ], capture_output=True)
    ...
    # Print logs
    r = subprocess.run(["docker", "logs", "bench-entdb", "--tail", "20"], capture_output=True, text=True)
...
def bench_entdb_reads(num: int) -> dict:
    ...
    for i in range(min(num, 50)):
        ...
        try:
            resp = stub.ExecuteAtomic(entdb_pb2.ExecuteAtomicRequest(
                context=ctx,
    ...
    for i in range(num):
        ...
        req = entdb_pb2.GetNodeRequest(
            context=ctx, type_id=1, node_id=node_ids[i % len(node_ids)],
        )
        start = time.perf_counter()
        try:
            stub.GetNode(req)
...
def bench_pg_writes(num: int) -> dict:
    ...
    conn = psycopg2.connect(host="localhost", port=5434, user="bench", password="bench", dbname="bench")
...
def bench_pg_reads(num: int) -> dict:
    ...
    for i in range(min(num, 50)):
        cur.execute(
            "INSERT INTO nodes VALUES (%s,%s,%s,%s,%s,%s,%s,%s)",
            (tid, f"node-{i}", 1, json.dumps({"name": f"Seed {i}"}),
             ts, ts, "user", "[]"),
        )
    conn.commit()
...
def main():
    ...
    for tier in TIERS:
        ...
        for acks in ACKS_MODES:
            ...
            if not start_entdb(tier, acks=acks):
                ...
                print(f"  {ew['throughput']:>7.1f}/sec  p50={ew['p50_ms']:.2f}ms  p99={ew['p99_ms']:.2f}ms")

                print("    reads... ", end="", flush=True)
                er = bench_entdb_reads(NUM_READS)
                tier_result[f"{label}_reads"] = er
                print(f"  {er['throughput']:>7.1f}/sec  p50={er['p50_ms']:.2f}ms  p99={er['p99_ms']:.2f}ms")
        ...
        if not start_postgres(tier):
            ...
            try:
                ...
                print(f"  {pw['throughput']:>7.1f}/sec  p50={pw['p50_ms']:.2f}ms  p99={pw['p99_ms']:.2f}ms")

                print("    reads... ", end="", flush=True)
                pr = bench_pg_reads(NUM_READS)
                tier_result["pg_reads"] = pr
                print(f"  {pr['throughput']:>7.1f}/sec  p50={pr['p50_ms']:.2f}ms  p99={pr['p99_ms']:.2f}ms")
    ...
    print(f"{'='*95}")
    print(f"{'Tier':<22} {'acks=all':>12} {'acks=1':>12} {'Postgres':>12}  {'all p50':>8} {'1 p50':>8} {'PG p50':>8}")
    print("-" * 95)
    for r in results:
    ...
    with open("/Users/arun/projects/opensource/tenant-shard-db/tests/benchmarks/fair-compare-results.json", "w") as f:
